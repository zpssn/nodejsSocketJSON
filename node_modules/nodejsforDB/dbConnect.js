...
function creatdataTable(conn,tablename,callback){
  conn.query('CREATE TABLE '+tablename+' (SN varchar(255),MT varchar(255),MW varchar(255),OT varchar(255),OW varchar(255),LT varchar(255),SF varchar(255),O2 varchar(255),TIME TIMESTAMP NULL DEFAULT now())',function(err,result){
    if(err){
...
var  dataGetSqlSNandTIME = 'SELECT * FROM normaldata where SN = ? AND TIME >=? AND TIME <=?';
//查 query
function querydataBySNandTimeFun(conn,SN,start_time,end_time,callback){
  conn.query(dataGetSqlSNandTIME,[SN,start_time,end_time],function (err, result) {
          if(err){
...
var  normaldataAddSql = 'INSERT INTO normaldata(SN,MT,MW,OT,OW,LT,SF,O2) VALUES(?,?,?,?,?,?,?,?)';
function insertnormaldataFun(conn,SN,MT,MW,OT,OW,LT,SF,O2,callback){
  conn.query(normaldataAddSql,[SN,MT,MW,OT,OW,LT,SF,O2],function (err, result) {
          if(err){
           console.log('[INSERT ERROR] - ',err.message);
...
exports.insertnormaldataFun = insertnormaldataFun;
exports.querydataBySNandTimeFun = querydataBySNandTimeFun;
